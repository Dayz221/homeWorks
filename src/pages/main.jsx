...
export default () => {
  ...
  const getGradient = (r1, g1, b1, r2, g2, b2, k) => {
    const r = r1 + (r2-r1)*k
    const g = g1 + (g2-g1)*k
    const b = b1 + (b2-b1)*k
    return `${r},${g},${b}`
      ...
      <div className="body_container">
          {/* <div className="main_menu">
            <div className="user_info_container">
              ...
              <div className="user_data">
                <div className="user_full_name">{user.last_name + " " + user.first_name}</div>
                <div className="user_tg_name">{"@" + user.username}</div>
              </div>
            </div>
            <div className="menu_data">
              <div className="tasks_count" style={{ 
                  borderColor: `rgb(${getGradient(255,68,35, 112,255,105, tasks.length? numOfCompletedTasks/tasks.length : 1)})`, 
                  backgroundColor: `rgba(${getGradient(255,68,35, 112,255,105, tasks.length? numOfCompletedTasks/tasks.length : 1)}, .4)` 
                }}>
                { numOfCompletedTasks + ' / ' + tasks.length }
              </div>

              <input type="text" className="task_search" placeholder="Поиск по заданиям"/>
            </div>
...

          {
            renderType == 0 ?
              ...
              :
              <div style={{margin: "20px 0", textAlign: "center"}}>Эта страничка жестко разрабатывается...</div>
          }
